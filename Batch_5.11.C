...
void Batch_5_11(int Run_Number, int Read_File = 0) {
  if (Read_File == 0) {
    // gROOT->Clear();
    gROOT->SetBatch(1);
    ...
    while (i < nentries) {
      ...
      // }
      cout << i << endl;
      Event_Display_5_9(Run_Number, i, 0, 0, 0, 0, 1);
      // fChain.GetEntry(i);
      // for (int i = 0; i < 256; ++i) {
    ...
    while (fdat_read.good()) {
      ...
      fdat_read >> Read_ievt;
      cout << Read_ievt << endl;
      Event_Display_5_9(Run_Number, Read_ievt, 0, 0, 0, 0, 1);
    }
  }
}
