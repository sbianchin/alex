...
void Event_Display_5_2(Int_t Run_Number=5, Int_t ievt=0, int walk=0, Int_t enable_cout=0, Int_t display = 0){ 
  ...
  TF1 *gr_kaon_fit = new TF1("test1", "pol1");
  double a_fit_kaon = 0.;
  double b_fit_kaon = 0.;
  //int a_fit_kaon = 0;
  //int b_fit_kaon = 0;
    ...
    if(TARGET_High_has_neighbours[i]){
      if(ADC_High_TARGET[i]>Angle_ADC_cut && has_TDC_hit[i]){      
        if(!k_stop_bar[i]){
          ...
          vec_xx_lepton_test.push_back(Xloc[i]);
          vec_yy_lepton_test.push_back(Yloc[i]);
        ...
        if(IsIn(i,channel[gap_to_fit-1][0], channel[gap_to_fit-1][1],   // Additional weight for fibers close to the edge if hit
          ...
          if(!k_stop_bar[i]){ 
            ...
            vec_lepton_size.push_back(Xloc[i]);
            //cout << "NIM2 : " << i << endl;
            //cout << "//// EDGE FIBERS : " << vec_xx_lepton.size() << endl;
          }
        }
      }
 ...
 // cout << "//// TOF1 : " << vec_xx_lepton.size() << endl;
  ...
  for(int i = 0; i<3; i++){
      vec_xx_lepton_test.push_back(Gap[gap_to_fit-1][1][0]);
      vec_yy_lepton_test.push_back(Gap[gap_to_fit-1][1][1]);
  }
  ...
  TGraph *gr_test_lepton = new TGraph(vec_xx_lepton_test.size(),&vec_xx_lepton_test[0],&vec_yy_lepton_test[0]);
  TGraph *gr_test_kaon = new TGraph(vec_xx_kaon.size(),&vec_xx_kaon[0],&vec_yy_kaon[0]);
  
  //cout << "TEST ALBANE : " << vec_xx_lepton_test.size() << endl;
  //cout << "TEST ALBANE 2 : " << vec_xx_kaon.size() << endl;


  //TF1 *gr_Leptons_fit;
  gr_test_lepton->SetMarkerStyle(21);                           gr_test_kaon->SetMarkerStyle(21);
  gr_test_lepton->SetMarkerColor(2);                            gr_test_kaon->SetMarkerColor(kBlue-6);
  gr_test_lepton->SetMarkerSize(0.8);                           gr_test_kaon->SetMarkerSize(0.8);
  gr_test_lepton->GetXaxis()->SetLimits(-50.,50.);              gr_test_kaon->GetXaxis()->SetLimits(-50.,50.);
  gr_test_lepton->GetYaxis()->SetRangeUser(-50.,50.);           gr_test_kaon->GetYaxis()->SetRangeUser(-50.,50.);


  TF1 *gr_test_lepton_fit = new TF1("gr_test_lepton_fit", "pol1");   
  gr_test_lepton_fit->SetParameter(0,1.0);
  gr_test_lepton_fit->SetParameter(1,1.0);  
  TF1 *gr_test_kaon_fit = new TF1("gr_test_kaon_fit", "pol1");
  double a_test_lepton_fit = 0.;       double a_test_kaon_fit = 0.;

  if(vec_xx_lepton.size()>0){
    //gr_test_lepton->Fit("pol1","QC");
    gr_test_lepton->Fit("gr_test_lepton_fit","QC");
    gr_test_lepton_fit = gr_test_lepton->GetFunction("gr_test_lepton_fit");
    gr_test_lepton_fit->SetLineWidth(2);
    gr_test_lepton_fit->SetLineColor(2);
    a_test_lepton_fit = gr_test_lepton_fit->GetParameter(1);
    cout << "The test lepton fit parameters are: " << a_test_lepton_fit << ", " << gr_test_lepton_fit->GetParameter(0) << "\n\n\n\n\n\n\n\n\n"<<endl;
  }

  if(vec_xx_kaon.size()>0){
    gr_test_kaon->Fit("pol1","QC");
    //gr_test_kaon->Fit("gr_test_kaon_fit","QC");
    gr_test_kaon_fit = gr_test_kaon->GetFunction("pol1");
    gr_test_kaon_fit->SetLineWidth(2);
    gr_test_kaon_fit->SetLineColor(4);
    a_test_kaon_fit = gr_test_kaon_fit->GetParameter(1);
  }

  TCanvas *c_TEST;
  c_TEST = new TCanvas("TEST","TEST",0,200,800,400);
  ...
  if(a_test_lepton_fit/a_test_kaon_fit<0){
  ...
  TF1 *gr_Leptons_fit = new TF1("gr_Leptons_fit", "pol1");

  gr_Leptons->SetLineColor(2);
  gr_Leptons->SetLineWidth(4);
  gr_Leptons->SetMarkerColor(4);
  gr_Leptons->SetMarkerStyle(21);
  gr_Leptons->GetXaxis()->SetLimits(-50.,50.);
  gr_Leptons->GetYaxis()->SetRangeUser(-50.,50.);
  //gr_Leptons->Fit("pol1","QCM");
  gr_Leptons->Fit("pol1","QC");
  gr_Leptons_fit = gr_Leptons->GetFunction("pol1");
  a_gr_Leptons = gr_Leptons_fit->GetParameter(1);
  b_gr_Leptons = gr_Leptons_fit->GetParameter(0);

  a_fit_TDC_Gap_Fibers = a_gr_Leptons;
  b_fit_TDC_Gap_Fibers = b_gr_Leptons;

  //cout << "ALBANE 1 : " << a_gr_Leptons << "   " << b_gr_Leptons << endl;
  ...
  vec_yy_kaon.clear();      vec_ey_kaon.clear();
    ...
    if(vec_xx_kaon.size()>0){
      ...
      gr_kaon_final_fit->SetParameter(0,1.0);
      gr_kaon_final_fit->SetParameter(1,1.0);
      Int_t r = gr_kaon_final->Fit("test1","Q");     
      ...
      //If fit fails, refit with error bars = 0
      if (a_fit_final_kaon > 100) { //Refit if fit is bad (when slope is very high)
         TGraph *gr_kaon_refit = new TGraphErrors(vec_xx_kaon.size(),&vec_xx_kaon[0],&vec_yy_kaon[0],
      0,0);
	r = gr_kaon_refit->Fit("test1","Q");
        gr_kaon_final_fit = gr_kaon_refit->GetFunction("test1");
        a_fit_final_kaon = gr_kaon_final_fit->GetParameter(1);
        b_fit_final_kaon = gr_kaon_final_fit->GetParameter(0);
	gr_kaon_final_fit->SetParameter(1,a_fit_final_kaon);
        gr_kaon_final_fit->SetParameter(0,b_fit_final_kaon);
	r = gr_kaon_final->Fit("test1","Q");
	gr_kaon_final_fit = gr_kaon_final->GetFunction("test1");
        //gr_kaon_final_fit = gr_kaon_final->GetFunction("pol1");
        gr_kaon_final_fit->SetLineWidth(2);
        gr_kaon_final_fit->SetLineColor(4);
  ...
  for(int i = 0; i<256; i++){
    //cout << "ALBANE : " << i << "  " << distance_to_line(Xloc[i],Yloc[i],a_fit_TDC_Gap_Fibers,b_fit_TDC_Gap_Fibers) << "  " << endl;
    if(distance_to_line(Xloc[i],Yloc[i],a_fit_TDC_Gap_Fibers,b_fit_TDC_Gap_Fibers) <= max_dist && TARGET_High_has_neighbours[i] && !k_stop_bar[i]){   
      if(ADC_High_TARGET[i]>Angle_ADC_cut && has_TDC_hit[i]){
          vec_xx_lepton.push_back(Xloc[i]);
          vec_ex_lepton.push_back(1.6);

          vec_yy_lepton.push_back(Yloc[i]);
          vec_ey_lepton.push_back(1.6);

          vec_xx_lepton_rotate.push_back(Xloc[TARGET_Rotated_index[i]]);
          vec_ex_lepton_rotate.push_back(1.6);

          vec_yy_lepton_rotate.push_back(Yloc[TARGET_Rotated_index[i]]);
          vec_ey_lepton_rotate.push_back(1.6);
  ...
  //cout << "GR2 : " << vec_xx_lepton.size() << endl;
  if(vec_xx_lepton.size() != 0){
    //TGraph *gr2_Leptons = new TGraph(vec_xx_lepton.size(),&vec_xx_lepton[0],&vec_yy_lepton[0]);
    TGraph *gr2_Leptons = new TGraphErrors(vec_xx_lepton.size(),&vec_xx_lepton[0],&vec_yy_lepton[0],
      &vec_ex_lepton[0],&vec_ey_lepton[0]);
    TF1 *gr_Leptons_fit2 = new TF1("gr_Leptons_fit2", "pol1");

    gr2_Leptons->SetMarkerStyle(25);
    gr2_Leptons->SetMarkerColor(2);
    gr2_Leptons->SetMarkerSize(0.8);
    gr2_Leptons->GetXaxis()->SetLimits(-50.,50.);
    gr2_Leptons->GetYaxis()->SetRangeUser(-50.,50.);
    //gr2_Leptons->Fit("pol1","QCM");
    gr2_Leptons->Fit("pol1","QC");
    gr_Leptons_fit2 = gr2_Leptons->GetFunction("pol1");
    a_gr_Leptons = gr_Leptons_fit2->GetParameter(1);
    b_gr_Leptons = gr_Leptons_fit2->GetParameter(0);
  }
  ...
  for(int i = 0; i<3; i++){
    ...
    vec_ex_lepton.push_back(0.);

    vec_yy_lepton.push_back(Gap[gap_to_fit-1][selected_TDC_selected][1]);
    vec_ey_lepton.push_back(0.);

    vec_bar.push_back(999);
  }
  ...
  //cout << "GR3 : " << vec_xx_lepton.size() << endl;
  TGraphErrors *gr3_Leptons = new TGraphErrors(vec_xx_lepton.size(),&vec_xx_lepton[0],&vec_yy_lepton[0],
    &vec_ex_lepton[0],&vec_ey_lepton[0]);
  //TGraph *gr3_Leptons = new TGraph(vec_xx_lepton.size(),&vec_xx_lepton[0],&vec_yy_lepton[0]);

  //TGraph *gr3_Leptons_test = new TGraph(vec_xx_lepton_test.size(),&vec_xx_lepton_test[0],&vec_yy_lepton_test[0]);
  TF1 *gr_Leptons_fit3 = new TF1("gr_Leptons_fit3", "pol1");
  gr_Leptons_fit3->SetParameter(1,1.0);
  gr_Leptons_fit3->SetParameter(0,1.0);
  gr3_Leptons->SetMarkerStyle(25);                    //gr3_Leptons_test->SetMarkerStyle(25);
  gr3_Leptons->SetMarkerColor(2);                     //gr3_Leptons_test->SetMarkerColor(5);
  gr3_Leptons->SetMarkerSize(0.8);                    //gr3_Leptons_test->SetMarkerSize(0.8);
  gr3_Leptons->GetXaxis()->SetLimits(-50.,50.);       //gr3_Leptons_test->GetXaxis()->SetLimits(-50.,50.);
  gr3_Leptons->GetYaxis()->SetRangeUser(-50.,50.);    //gr3_Leptons_test->GetYaxis()->SetRangeUser(-50.,50.);
  //gr3_Leptons->Fit("pol1","QCM");

  Int_t r = gr3_Leptons->Fit("gr_Leptons_fit3","V");
  gr_Leptons_fit3 = gr3_Leptons->GetFunction("gr_Leptons_fit3");
  gr_Leptons_fit3->SetLineWidth(2);
  gr_Leptons_fit3->SetLineColor(2);

  a_gr_Leptons = gr_Leptons_fit3->GetParameter(1);
  b_gr_Leptons = gr_Leptons_fit3->GetParameter(0);

  if (a_gr_Leptons > 100) { //Refit if fit is bad (when slope is very high)
       TGraph *gr_Leptons_refit = new TGraphErrors(vec_xx_lepton.size(),&vec_xx_lepton[0],&vec_yy_lepton[0],
      0,0);

	r = gr_Leptons_refit->Fit("gr_Leptons_fit3","Q");
        gr_Leptons_fit3 = gr_Leptons_refit->GetFunction("gr_Leptons_fit3");
        a_gr_Leptons = gr_Leptons_fit3->GetParameter(1);
        b_gr_Leptons = gr_Leptons_fit3->GetParameter(0);
	gr_Leptons_fit3->SetParameter(1,a_gr_Leptons);
        gr_Leptons_fit3->SetParameter(0,b_gr_Leptons);
	gr_Leptons_fit3 = gr3_Leptons->GetFunction("test1");
        a_gr_Leptons = gr_Leptons_fit3->GetParameter(1);
        b_gr_Leptons = gr_Leptons_fit3->GetParameter(0);
      } ;
  
  ParError=gr_Leptons_fit3->GetParError(1);
  ChiS = gr_Leptons_fit3->GetChisquare();
  ndf = gr_Leptons_fit3->GetNDF();

  //TGraph *gr3_Leptons_rotate = new TGraph(vec_xx_lepton_rotate.size(),&vec_xx_lepton_rotate[0],&vec_yy_lepton_rotate[0]);
  ...
  gr3_Leptons_rotate->SetMarkerStyle(25);
  ...
  cout << "NDF-w(2) = " << ndf-2 << endl; 
  cout << "Reduced Chi Square = " << ChiS/(ndf-2) << endl;
  cout << "" << endl;
  cout << "" << endl;
  ...
  a_fit_lepton_rotate = -1/a_fit_TDC_Gap_Fibers;
...
//  }
  ...
  if((gap_to_fit_rotate==6 || gap_to_fit_rotate==12 || selected_TOF2==12 || selected_TOF2==6) && Rotate==1){
    a_gr_Leptons = a_fit_lepton_rotate;
  }
  else{
    a_gr_Leptons = a_fit_TDC_Gap_Fibers;
  }
  ...
  if((gap_to_fit_rotate==12 || gap_to_fit_rotate==6 || selected_TOF2==12 || selected_TOF2==6) && Rotate==1){ //ROTATE_CHANGE
    vec_test = _2lines_intersect(-1/a_fit_TDC_Gap_Fibers, -b_fit_TDC_Gap_Fibers/a_fit_TDC_Gap_Fibers, a_fit_kaon, b_fit_kaon);
    ...
    //if(vec_xx_kaon.size()>0) gr_kaon_final->Draw("P");
    if(vec_xx_kaon.size()>0) gr_kaon_final->Draw("P");
    ...
    //k_stop_line->Draw("same");  
      ...
      //cout << "ALBANE :  " << a_gr_Leptons << "  ";
      //cout << b_gr_Leptons << "  ";
      //cout << a_fit_kaon << "  ";
      //cout << b_fit_kaon << endl;

      vec_test = _2lines_intersect(a_gr_Leptons, b_gr_Leptons, a_fit_kaon, b_fit_kaon);
      h_2lines_intersect->Fill(vec_test[0],vec_test[1]);
  ...
  if(display != 0){
    ...
    if((gap_to_fit_rotate==6 || gap_to_fit_rotate==12 || selected_TOF2==12 || selected_TOF2==6) && Rotate==1){
      ...
      for(unsigned j=0; j<vec_xx_lepton_rotate.size(); j++){
        vec_yprime_rotate.push_back(-a_fit_TDC_Gap_Fibers*vec_yy_lepton_rotate[j] - b_fit_TDC_Gap_Fibers);
        vec_Dy_rotate.push_back(vec_xx_lepton_rotate[j] - (-a_fit_TDC_Gap_Fibers*vec_yy_lepton_rotate[j] - b_fit_TDC_Gap_Fibers)); 
      }
     
      cout << "y = mx + b  with:  m = ";
      cout << setw(7) << setprecision(3) << a_fit_TDC_Gap_Fibers;
      cout << " and b = " << setw(8) << setprecision(3) << b_fit_TDC_Gap_Fibers << endl;
      cout << endl;
      ...
      for(unsigned j=0; j<vec_xx_lepton.size(); j++){
        vec_yprime.push_back(a_fit_TDC_Gap_Fibers*vec_xx_lepton[j] + b_fit_TDC_Gap_Fibers);
        vec_Dy.push_back(vec_yy_lepton[j] - (a_fit_TDC_Gap_Fibers*vec_xx_lepton[j] + b_fit_TDC_Gap_Fibers)); 
      }
