...
TrackVariables FindTrack(vector<int> lepton_hit_bars, double dv_x, double dv_y,
  ...
  for (std::size_t i = 0; i < lepton_hit_bars.size(); ++i) {
  ...
  for (std::size_t i = 0; i < vec_l1.size(); ++i) already_assigned[vec_l1[i]] = true;
  for (std::size_t i = 0; i < vec_l2.size(); ++i) already_assigned[vec_l2[i]] = true;
  for (std::size_t i = 0; i < vec_l1.size(); ++i) {
      ...
      for (std::size_t neighbor = 0; neighbor < neighbors.size(); ++neighbor) {
        if (already_assigned[neighbors[neighbor]] == false) {
              ...
              for (std::size_t j = 0; j < adj_node_neighbors.size(); ++j) {
