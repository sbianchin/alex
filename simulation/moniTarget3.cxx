...
void moniTarget3(){
	...
	target->SetXTitle("X-axis (mm)");
	target->SetYTitle("Y-axis (mm)");
	TH2F *time_h2f=new TH2F("time","time",20, 0, 20, 20, 0, 20);
	time_h2f->SetXTitle("X-axis (bar #)");
	time_h2f->SetYTitle("Y-axis (bar #)");

        char output[100]="target824.pdf";
	...
	*/
	...
	for(int i=0;i<6;i++){  targX[m]=12-i; targY[m]=1; m++;}
        */
	TFile* file=new TFile("G4Run824.root");
	TTree* t1=(TTree*)file->Get("Kaon");
  ...
  // for(int j=1;j<3;j++){
	...
	for(int i=0;i<256;i++){
		...
		time_h2f->Fill(targX[i],targY[i],targetdt[i]);
	}
	c1->cd(1);
