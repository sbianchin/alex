// modify dX, dY calculations lines 3045,3046  to use Delta_phi in RADIANS
...
//void Event_Display_5_5(int Run_Number=3994, int ievt=0, int enable_cout=0, int Switch_Printout = 1,
//  int Switch_Display = 1, int print_output = 1, int Read_File = 0, int batch=0){

void Event_Display_6_0(int Run_Number=3994, int ievt=0, int enable_cout=0, int Switch_Printout = 1,
  ...
  char Version[100] = "Version 6.0";  // TO MOVE
...
*/
  ...
  if(!Good_Event){
    if(batch==1){
      if(batch_flag==0 || batch_flag==2 || batch_flag==3) fout << endl;
...
*/
  ...
  if(vec_xx_kaon.size()>=kaon_cluster_size){
    vertical_kaon = check_vertical_kaon(vec_xx_kaon);
    if (!vertical_kaon) {
...
/*  else{
    if(batch==0 && Good_Event){
      cout << endl;
      cout << "TOO FEW KAON HITS !   ( < " << kaon_cluster_size << " ) " << endl;
      cout << endl;
      return;
    }
    if(batch==1) return;
...
*/
  ...
  To_rotate = false;
  if(gap_to_fit==5 || gap_to_fit==6 || gap_to_fit==7 || gap_to_fit==11 || gap_to_fit==12 || gap_to_fit==1) To_rotate = true;
...
*/
    if(To_rotate){
      vec_xx_lepton.push_back(Gap[TOF1_rotated[gap_to_fit-1]][1][0]);
      vec_ex_lepton.push_back(TOF1_Errors_X[2][1]);

      vec_yy_lepton.push_back(Gap[TOF1_rotated[gap_to_fit-1]][1][1]);
      vec_ey_lepton.push_back(TOF1_Errors_Y[2][1]);
    }

/*    else if(gap_to_fit == 11){
      vec_xx_lepton.push_back(Gap[7][1][0]);
      vec_ex_lepton.push_back(TOF1_Errors_X[2][1]);

      vec_yy_lepton.push_back(Gap[7][1][1]);
      vec_ey_lepton.push_back(TOF1_Errors_Y[2][1]);
    }
*/    else{
  ...
  if(vec_xx_lepton.size()>3){
    ...
    //gr_lepton_1->Fit("Lepton_fit_1","QW");
...
*/
  ...
  //for(unsigned int i=0; i<vec_Cpi.size(); i++) cout << "TEST :" << vec_Cpi[i] << endl;
  //cout << "SUM : " << TDC_cpi_sum << "  " << vec_Cpi.size() << "  " << double(TDC_cpi_sum)/double(vec_Cpi.size()) << endl;
...
*/
  ...
  vector<int> lepton_hit_bars_two_track; //contains lepton bar ids for two-track fitting
    ...
    for(unsigned int i=0; i<vec_lepton_bars.size(); i++){
      if(distance_to_line(Xloc[TARGET_Rotated_index[vec_lepton_bars[i]]],Yloc[TARGET_Rotated_index[vec_lepton_bars[i]]],a_lepton_fit_1,b_lepton_fit_1) <= max_dist
        ...
        if(ADC_High_TARGET[vec_lepton_bars[i]]>Angle_ADC_cut && has_TDC_hit[vec_lepton_bars[i]]){
          ...
          lepton_hit_bars_two_track.push_back(vec_lepton_bars[i]);
...
/*          if(gap_to_fit == 6){
            if(IsIn(TARGET_Rotated_index[vec_lepton_bars[i]],channel[3-1][0], channel[3-1][1],
              ...
              for(int iw=0; iw<weight_edge_bars_fit2; iw++){
                vec_xx_lepton.push_back(Xloc[TARGET_Rotated_index[vec_lepton_bars[i]]]);
                vec_ex_lepton.push_back(TARGET_Errors_X);
                ...
                cout << "TEST : " << i << "  "
                     << vec_lepton_bars[i] << "   "
                     << TARGET_Rotated_index[vec_lepton_bars[i]] << "   "
                     << Xloc[TARGET_Rotated_index[vec_lepton_bars[i]]] << "  "
                     << Yloc[TARGET_Rotated_index[vec_lepton_bars[i]]] << endl;
                cout << endl;
                cout << endl;
...
*/
          //if(To_rotate){
            if(IsIn(TARGET_Rotated_index[vec_lepton_bars[i]], channel[TOF1_rotated[gap_to_fit-1]][0], channel[TOF1_rotated[gap_to_fit-1]][1],
                      channel[TOF1_rotated[gap_to_fit-1]][2], channel[TOF1_rotated[gap_to_fit-1]][3],
                      channel[TOF1_rotated[gap_to_fit-1]][4], channel[TOF1_rotated[gap_to_fit-1]][5],
                      channel[TOF1_rotated[gap_to_fit-1]][6], channel[TOF1_rotated[gap_to_fit-1]][7])){
              for(int iw=0; iw<weight_edge_bars_fit2; iw++){
                vec_xx_lepton.push_back(Xloc[TARGET_Rotated_index[vec_lepton_bars[i]]]);
                vec_ex_lepton.push_back(TARGET_Errors_X);
...
*/        }
  ...
  if(vec_xx_lepton.size()>0){
    ...
    //gr_lepton_2->Fit("lepton_fit_2","QW");
    gr_lepton_2->Fit("lepton_fit_2","QB");
    delete func_lepton_fit_2;
    func_lepton_fit_2_ptr = gr_lepton_2->GetFunction("lepton_fit_2");
    ...
    //if(batch==0) cout << "TOO FEW LEPTONS !" << endl;
    if(batch==1){
      if(batch_flag==0 || batch_flag==2 || batch_flag==3) fout << endl;
      if(batch_flag==1) cout << endl;
      return;
...
*/
    if(To_rotate){
      dist1_GoodLG[i] = distance(x_int_GoodLG[i], y_int_GoodLG[i], Gap[TOF1_rotated[gap_to_fit-1]][1][0], Gap[TOF1_rotated[gap_to_fit-1]][1][1]);
      dist1_TDC_Gap_Fibers[i] = distance(x_int_TDC_Gap_Fibers[i], y_int_TDC_Gap_Fibers[i], Gap[TOF1_rotated[gap_to_fit-1]][1][0], Gap[TOF1_rotated[gap_to_fit-1]][1][1]);
      dist1_TDC_Gap_Fibers_SFT[i] = distance(x_int_TDC_Gap_Fibers_SFT[i], y_int_TDC_Gap_Fibers_SFT[i], Gap[TOF1_rotated[gap_to_fit-1]][1][0], Gap[TOF1_rotated[gap_to_fit-1]][1][1]);
    }
    else{
...
*/
    if(To_rotate){
      dist2_TDC_selected[ii] = distance(x_TDC_Gap_Fibers, y_TDC_Gap_Fibers, Gap[TOF1_rotated[gap_to_fit-1]][ii][0], Gap[TOF1_rotated[gap_to_fit-1]][ii][1]);
      dist2_GoodLG[ii] = distance(x_GoodLG_intersect1, y_GoodLG_intersect1, Gap[TOF1_rotated[gap_to_fit-1]][ii][0], Gap[TOF1_rotated[gap_to_fit-1]][ii][1]);
     }
    else{
  ...
  vec_xx_TOF1_closest.push_back(Gap[gap_to_fit-1][selected_TDC_selected][0]); // TO MOVE
  vec_yy_TOF1_closest.push_back(Gap[gap_to_fit-1][selected_TDC_selected][1]); // TO MOVE
...
/*    if(gap_to_fit == 6){
      ...
      vec_xx_lepton_rotate.push_back(Gap[gap_to_fit-1][selected_TDC_selected][0]);
      vec_ex_lepton_rotate.push_back(TOF1_Errors_X[gap_to_fit-1][selected_TDC_selected]);

      vec_yy_lepton.push_back(Gap[3-1][selected_TDC_selected][1]);
      vec_ey_lepton.push_back(TOF1_Errors_Y[3-1][selected_TDC_selected]);
      vec_yy_lepton_rotate.push_back(Gap[gap_to_fit-1][selected_TDC_selected][1]);
      vec_ey_lepton_rotate.push_back(TOF1_Errors_Y[gap_to_fit-1][selected_TDC_selected]);
    }
 */
    //else if(gap_to_fit == 12){
/*    if(gap_to_fit == 12){
...
*/
    if(To_rotate){
      vec_xx_lepton.push_back(Gap[TOF1_rotated[gap_to_fit-1]][selected_TDC_selected][0]);
      vec_ex_lepton.push_back(TOF1_Errors_X[TOF1_rotated[gap_to_fit-1]][selected_TDC_selected]);
      vec_xx_lepton_rotate.push_back(Gap[gap_to_fit-1][selected_TDC_selected][0]);
      vec_ex_lepton_rotate.push_back(TOF1_Errors_X[gap_to_fit-1][selected_TDC_selected]);

      vec_yy_lepton.push_back(Gap[TOF1_rotated[gap_to_fit-1]][selected_TDC_selected][1]);
      vec_ey_lepton.push_back(TOF1_Errors_Y[TOF1_rotated[gap_to_fit-1]][selected_TDC_selected]);
      vec_yy_lepton_rotate.push_back(Gap[gap_to_fit-1][selected_TDC_selected][1]);
      vec_ey_lepton_rotate.push_back(TOF1_Errors_Y[gap_to_fit-1][selected_TDC_selected]);
    }
    else{
...
*/
  gr_lepton_3->Fit("lepton_fit_3","QW");
...
*/
  ...
  double a_lepton_fit_3_error = func_lepton_fit_3_ptr->GetParError(1);
  double b_lepton_fit_3_error = func_lepton_fit_3_ptr->GetParError(0);
...
*/
    if(To_rotate){
      dist1_TDC_Gap_Fibers[i] = distance(x_int_TDC_Gap_Fibers[i], y_int_TDC_Gap_Fibers[i], Gap[TOF1_rotated[gap_to_fit-1]][1][0], Gap[TOF1_rotated[gap_to_fit-1]][1][1]);
      dist1_TDC_Gap_Fibers_SFT[i] = distance(x_int_TDC_Gap_Fibers_SFT[i], y_int_TDC_Gap_Fibers_SFT[i], Gap[TOF1_rotated[gap_to_fit-1]][1][0], Gap[TOF1_rotated[gap_to_fit-1]][1][1]);
      dist1_TARGET_intersect[i] = distance(x_int_TARGET[i], y_int_TARGET[i], Gap[TOF1_rotated[gap_to_fit-1]][1][0], Gap[TOF1_rotated[gap_to_fit-1]][1][1]);
    }
    else{
      dist1_TDC_Gap_Fibers[i] = distance(x_int_TDC_Gap_Fibers[i], y_int_TDC_Gap_Fibers[i], Gap[gap_to_fit-1][1][0], Gap[gap_to_fit-1][1][1]);
      dist1_TDC_Gap_Fibers_SFT[i] = distance(x_int_TDC_Gap_Fibers_SFT[i], y_int_TDC_Gap_Fibers_SFT[i], Gap[gap_to_fit-1][1][0], Gap[gap_to_fit-1][1][1]);
  ...
  uncertainty d = calculate_uncertainty(x_TARGET_intersect, y_TARGET_intersect, a_lepton_fit_3,
    b_lepton_fit_3, a_lepton_fit_3_error, b_lepton_fit_3_error);
  double dx = d.dx;
  double dy = d.dy;
  ...
  if(To_rotate) a_final_guide = a_lepton_fit_3;
  ...
  //cout << "TEST : " << Delta_X << "    " << Delta_Y << endl;
...
*/
  ...
  //Delta_X = sqrt(pow(TARGET_Errors_X,2) + pow((sin(alpha_guide)*R_TARGET*Delta_phi_deg),2));
  //Delta_Y = sqrt(pow(TARGET_Errors_X,2) + pow((cos(alpha_guide)*R_TARGET*Delta_phi_deg),2));

  // change Delta_phi to RADIANS -- MH Oct 14/17

  Delta_X = sqrt(pow(TARGET_Errors_X,2) + pow((sin(angle_final_guide)*R_TARGET*Delta_phi),2));
  Delta_Y = sqrt(pow(TARGET_Errors_X,2) + pow((cos(angle_final_guide)*R_TARGET*Delta_phi),2));
...
*/
  ...
  if(vec_kaon_bars.size()>=kaon_cluster_size){
    if (vertical_kaon) {
      //if(gap_to_fit==6 || gap_to_fit==12){
      if(To_rotate){
        vec_fit_lines_intersect.push_back(vec_xx_kaon[0]);
        vec_fit_lines_intersect.push_back(a_fit_lepton_rotate * vec_xx_kaon[0] + b_fit_lepton_rotate);
      }
      else{
        vec_fit_lines_intersect.push_back(vec_xx_kaon[0]);
        vec_fit_lines_intersect.push_back(a_lepton_fit_3 * vec_xx_kaon[0] + b_lepton_fit_3);
      }
  ...
  //Fitting 2 tracks
  // FindTrack(vector<int> lepton_hit_bars, double dv_x, double dv_y,
  ...
  if (lepton_hit_bars_two_track.size() > 0) { //insert condition for two-track analysis
    // return;
    TrackVariables two_tracks = FindTrack(lepton_hit_bars_two_track, vec_xx_kaon_stop[0], vec_xx_kaon_stop[1], vec_xx_TOF1_closest[0], vec_yy_TOF1_closest[0], gap_to_fit-1);
    primary_track = two_tracks.primary_track;
    secondary_track = two_tracks.secondary_track;
  }
  ...
  //if(Switch_Printout !=0) {
    ...
    if(Switch_Printout !=0){
      cout << "Z_TOF1  =  " << Z_TOF1[gap_to_fit-1] << " mm" << endl;
    }
...
*/    }
    ...
    cout << endl;
    cout << "N(Ck) = " << vec_Ck.size() << "   " << "TDC Ck Avg. = " << TDC_ck_avg << "   " << "Var(Ck) = " << TDC_ck_sigma << "   " << "Sigma(Ck) = " << TDC_ck_sigma/sqrt(vec_Ck.size()) << endl;
    cout << "N(Cpi) =  " << vec_Cpi.size() << "   " << "TDC Cpi Avg. = " << TDC_cpi_avg << "   " << "Var(Cpi) = " << TDC_cpi_sigma << "   " << "Sigma(Cpi) = " << TDC_cpi_sigma/sqrt(vec_Cpi.size()) << endl;
    cout << endl;
    cout << "N'(Ck) = " << TDC_ck_counter << "   " << "TDC'(Ck) = " << TDC_ck_avg2 << endl;
    cout << "N'(Cpi) = " << TDC_cpi_counter << "   " << "TDC'(Cpi) = " << TDC_cpi_avg2 << endl;
    cout << endl;
  ...
  if(vec_xx_kaon.size()<5){
    ...
    Subplot_5(h_kaon_fit_final, A1,
      ...
      ell_L1,
      gr_kaon, gr_kaon_bk,
      vec_xx_kaon, kaon_bk, vertical_kaon);
    ...
    if (lepton_hit_bars_two_track.size() > 0) { //insert condition for two-track analysis
      TCanvas *c2_tracks;
      c2_tracks = new TCanvas("Event_Display.C  --  Two Track Fit","Event_Display.C  --  Two Track Fit",0,200,1050,700);
      c2_tracks->Divide(3,2);
      c2_tracks->Connect("ProcessedEvent(Int_t,Int_t,Int_t,TObject*)",0,0, "ProcEvent(Int_t,Int_t,Int_t,TObject*)");

      Two_tracks_Subplot_1(c2_tracks, Run_Number, ievt, gr_lepton_3,
        To_rotate,
        gr3_Leptons_rotate,
        best_fit_rotate,
        gap_to_fit);
      Two_tracks_Subplot_2(c2_tracks, gr_kaon, gr_kaon_bk,
      vec_xx_kaon, kaon_bk, vertical_kaon);
      Two_tracks_Subplot_3(c2_tracks, gr_lepton_3, To_rotate,
        gr3_Leptons_rotate,
        best_fit_rotate,
        gr_kaon, gr_kaon_bk,
        vec_xx_kaon, kaon_bk, vertical_kaon, gr_kaon_stop);
      const char plot_titleA[50] = "Primary Track";
      const char plot_titleB[50] = "Secondary Track";
      const char plot_titleAB[50] = "Both Tracks";
      Two_tracks_Subplot_4(c2_tracks, primary_track,
        gr_TOF1, gr_TOF1_closest, 4, plot_titleA, 2);
      Two_tracks_Subplot_4(c2_tracks, secondary_track,
        gr_TOF1, gr_TOF1_closest, 5, plot_titleB, 8);

      Two_tracks_Subplot_6(c2_tracks, primary_track, secondary_track,
      gr_TOF1, gr_TOF1_closest,
      gr_kaon_stop,
      plot_titleAB, 6, 2, 8);
      return;
  ...
  //if(Delta_phi_deg==0) cout << "TO CHECK :  " << ievt << endl;
  ...
  if(print_output != 0){
    ...
    cout << setw(6) << setprecision(2) << angle_final_guide << " ";
    cout << setw(6) << setprecision(2) << Delta_phi_deg << " ";
    cout << setw(6) << setprecision(2) << Delta_X << " ";
    cout << setw(6) << setprecision(2) << Delta_Y << " ";
    cout << setw(7) << setprecision(2) << Chis_lepton_fit_3 << " ";
    cout << setw(3) << setprecision(0) << ndf_lepton_fit_3 << " ";
    cout << setw(6) << setprecision(2) << Chis_lepton_fit_3/ndf_lepton_fit_3 << " ";
    cout << setw(3) << lepton_counter << "  ";
    cout << setw(6) << setprecision(2) << vec_xx_int_TDC_TARGET[0] << " ";
    cout << setw(6) << setprecision(2) << vec_yy_int_TDC_TARGET[0] << " " ;
    cout << setw(7) << setprecision(2) << vec_kaon_centroid_coordinates[0] << " ";
    cout << setw(7) << setprecision(2) << vec_kaon_centroid_coordinates[1] << " ";
    cout << setw(7) << setprecision(2) << vec_fit_lines_intersect[0] << " ";
    cout << setw(7) << setprecision(2) << vec_fit_lines_intersect[1] << " ";
    cout << setw(7) << setprecision(2) << vec_k_stop_coordinates[0] << " ";
    cout << setw(7) << setprecision(2) << vec_k_stop_coordinates[1] << " ";
  ...
  if(batch==1 && (batch_flag== 0 || batch_flag==2 || batch_flag==3)){ // {print_output
    ...
    fout << setw(6) << setprecision(2) << angle_final_guide << " ";
    fout << setw(6) << setprecision(2) << Delta_phi_deg << " ";
    fout << setw(6) << setprecision(2) << Delta_X << " ";
    fout << setw(6) << setprecision(2) << Delta_Y << " ";
    fout << setw(7) << setprecision(2) << Chis_lepton_fit_3 << " ";
    fout << setw(3) << setprecision(0) << ndf_lepton_fit_3 << " ";
    fout << setw(6) << setprecision(2) << Chis_lepton_fit_3/ndf_lepton_fit_3 << " ";
    fout << setw(3) << lepton_counter << "  ";
    fout << setw(6) << setprecision(2) << vec_xx_int_TDC_TARGET[0] << " ";
    fout << setw(6) << setprecision(2) << vec_yy_int_TDC_TARGET[0] << " " ;
    fout << setw(7) << setprecision(2) << vec_kaon_centroid_coordinates[0] << " ";
    fout << setw(7) << setprecision(2) << vec_kaon_centroid_coordinates[1] << " ";
    fout << setw(7) << setprecision(2) << vec_fit_lines_intersect[0] << " ";
    fout << setw(7) << setprecision(2) << vec_fit_lines_intersect[1] << " ";
    fout << setw(7) << setprecision(2) << vec_k_stop_coordinates[0] << " ";
    fout << setw(7) << setprecision(2) << vec_k_stop_coordinates[1] << " ";
  ...
  if(Zlist_flag){
    if(batch==1 && (batch_flag== 0 || batch_flag==2 || batch_flag==3)){ // {print_output
      ...
      fout2 << setw(7) << ievt << " ";
      for(unsigned int k=0; k<Z_selected.size(); k++){
        fout2 << setw(7) << setprecision(2) << Z_selected[k] << " ";
      }
      fout2 << endl;
      fout2.close();
    }
...
double SFT_Test_CR_Event(int Run_Number, int evt, double phi, bool to_print, double C2X_centroid, double length_in_target){
  ...
  //cout << "TEST TEST TEST : " << ifile << endl;
