...
{ 
  ...
  TFile hfile("angle_tree.root","RECREATE");
  TTree *angle_tree = new TTree("Event_Angles", "Event_Angles");

  angle_tree->Branch("Angle", &p_angle, "p_angle/F");
...
MWPC_L_R = 'R';
  ...
  for(Int_t imark1=0; imark1<15; imark1++){
    if((ADC_High_SFT_corr[par_temp[1][imark1]]!=0) && (has_TDC_SFT_hit[par_temp[1][imark1]]) || 
       (ADC_High_SFT_corr[par_temp[1][imark1+64]]!=0) && (has_TDC_SFT_hit[par_temp[1][imark1+64]])){
      SFT_L1_count++;
    }
  }
  ...
  for(Int_t imark2=0; imark2<15; imark2++){
    if((ADC_High_SFT_corr[par_temp[1][imark2+15]]!=0) && (has_TDC_SFT_hit[par_temp[1][imark2+15]]) ||
       (ADC_High_SFT_corr[par_temp[1][imark2+79]]!=0) && (has_TDC_SFT_hit[par_temp[1][imark2+79]])){
      SFT_L2_count++;
    }
  }
  ...
  for(Int_t imark3=0; imark3<17; imark3++){
    if((ADC_High_SFT_corr[par_temp[1][imark3+30]]!=0) && (has_TDC_SFT_hit[par_temp[1][imark3+30]]) ||
       (ADC_High_SFT_corr[par_temp[1][imark3+94]]!=0) && (has_TDC_SFT_hit[par_temp[1][imark3+94]])){
      SFT_L3_count++;
    }
  }
  ...
  for(Int_t imark4=0; imark4<17; imark4++){
    if((ADC_High_SFT_corr[par_temp[1][imark4+47]]!=0) && (has_TDC_SFT_hit[par_temp[1][imark4+47]]) ||
      (ADC_High_SFT_corr[par_temp[1][imark4+111]]!=0) && (has_TDC_SFT_hit[par_temp[1][imark4+111]])){
      SFT_L4_count++;
    }
  }  
...
for(Int_t i=0; i<256; i++){
  if(TARGET_High_has_neighbours[i]){
    ...
    //Fit 2: Targets hit within max_distance of the best fit line from Fit 1
      ...
      // *p_angle = angle_final_guide;
      // angle_tree->Fill(angle_final_guide); 

      if(angle_final_guide>85 && angle_final_guide < 95 && SFT_L1_count==1 && SFT_L2_count==1 && SFT_L3_count==1 && SFT_L4_count==1){
