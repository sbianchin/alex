...
void SFT_Efficiency(Int_t run_number=1710, Int_t NHit_TARGET_good_event=5, Int_t Nb_events=-1) {
...
//sprintf(par_finput,"/triumfcs/trshare/trek/E36/Macros/April_2015/Mapping/%s",source_mapping);
sprintf(par_finput5,"%s/ADC_TOF1_Thresholds.txt",par_map);

char par_finput6[200];
//sprintf(par_finput,"/triumfcs/trshare/trek/E36/Macros/April_2015/Mapping/%s",source_mapping);
sprintf(par_finput6,"%s/ADC_TOF2_Thresholds.txt",par_map);
...
Int_t par_temp_TOF2[2][48];
ifstream fdat6(par_finput6,ios::in);
for(Int_t ii=0; ii<48; ii++) fdat6 >> par_temp_TOF2[0][ii] >> par_temp_TOF2[1][ii];
fdat6.close();
...
//	fChain_SFT->GetEntry(j);
	...
	for(Int_t it=0; it<256; it++){
		if(ADC_High_target[it]>par_temp_TARGET[1][it]) count_target++;
	...
	if(count_target>=NHit_TARGET_good_event){
		good_event++;
...
///Layer 1
	for (Int_t i=0; i<15; i++) {
		if (((fiber[i]>0) && (fiberTDC[i]>TDCthr1) && (fiberTDC[i]<TDCthr2)) || ((fiberTDC[i+64]>TDCthr1) && (fiberTDC[i+64]<TDCthr2) && (fiber[i+64]>0))) {
			counter1++;
			if (counterp1 == 0) {
				counterp1 = i;
			}
...
///////////////////////////////////////////////////////////////////////// NO TDC CUT Efficiency
	...
	for (Int_t i=0; i<15; i++) {
		if (((fiber[i]>0) && (fiberTDC[i]>0) && (fiberTDC[i]<TDCthr2)) || ((fiberTDC[i+64]>0) && (fiberTDC[i+64]<TDCthr2) && (fiber[i+64]>0))) {
			counter1a++;
		}
		if (((fiber[i]>0) && (fiberTDC[i]>0) && (fiberTDC[i]<TDCthr2)) && ((fiberTDC[i+64]>0) && (fiberTDC[i+64]<TDCthr2) && (fiber[i+64]>0))) {
			counter5a++;
...
//	cout << "Hit_Down  " << "Hit_Up" << endl;
//	cout << layer1hitdown << "         " << layer1hitup << endl;
//	cout << layer2hitdown << "         " << layer2hitup << endl;
//	cout << layer3hitdown << "         " << layer3hitup << endl;
//	cout << layer4hitdown << "         " << layer4hitup << endl;
...
//		cout << "One Hit Per Layer: Event " << j << endl;
		//p1_p2 = counterp1-(counterp2-15);
		//if (p1_p2 <0) p1_p2 = p1_p2 + 15; 
		//p3_p4 = counterp3-(counterp4-17);
		//if (p3_p4 <0) p3_p4 = p3_p4 + 17; 

		p1_p2 = counterp2 - counterp1;
		if(p1_p2<0) p1_p2 = p1_p2 + 15;

		p3_p4 = counterp4 - counterp3;
		if(p3_p4<0){
			p3_p4 = p3_p4 + 17;
		}
