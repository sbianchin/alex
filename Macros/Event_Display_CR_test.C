...
void SFT_Test_CR_Event(int Run_Number, int evt, double phi_top, double phi_bottom, double C2_top, double C2_bottom, double SFT_Array[2], double ADC_High_SFT_corr[128], int has_TDC_SFT_hit[128]);

void Event_Display_CR_test(Int_t Run_Number=5, Int_t ievt=0, Int_t enable_cout=9, Int_t display_fits = 0){ 
 ...
 /////////////////////////////////////////////////////////////////
  ...
  for(int i = 0; i < c2xl_num_lines; i++){
    for(int j = 0; j < c2xr_num_lines; j++){
      ...
      double C2_intersect_SFT_top_temp = C2_intersect(points_to_slope(x1_C2_coord,y1_C2_coord,x2_C2_coord,y2_C2_coord),points_to_y_int(x1_C2_coord,y1_C2_coord,x2_C2_coord,y2_C2_coord),R_SFT_L1);
      double C2_intersect_SFT_bottom_temp = C2_intersect(points_to_slope(x1_C2_coord,y1_C2_coord,x2_C2_coord,y2_C2_coord),points_to_y_int(x1_C2_coord,y1_C2_coord,x2_C2_coord,y2_C2_coord),-R_SFT_L1);
      
      if(C2_intersect_SFT_top_temp > -135 && C2_intersect_SFT_top_temp < 135)
        C2_intersect_SFT_top = C2_intersect_SFT_top_temp;
      if(C2_intersect_SFT_bottom_temp > -135 && C2_intersect_SFT_bottom_temp < 135)
        C2_intersect_SFT_bottom = C2_intersect_SFT_bottom_temp;
  ...
  SFT_angle_selector(Run_Number,ievt,angle_final_guide, gap_to_fit_left, gap_to_fit_right, 
    C2_intersect_SFT_top, C2_intersect_SFT_bottom, SFT_Array, ADC_High_SFT_corr, has_TDC_SFT_hit);
...
void SFT_Test_CR_Event(int Run_Number, int evt, double phi_top, double phi_bottom, double C2_top, double C2_bottom, double SFT_Array[2], double ADC_High_SFT_corr[128], int has_TDC_SFT_hit[128]){
  SFT_print(ADC_High_SFT_corr, has_TDC_SFT_hit, SFT_channel_to_fiber, evt, phi_top, phi_bottom, C2_top, C2_bottom, SFT_Array, true);
}
