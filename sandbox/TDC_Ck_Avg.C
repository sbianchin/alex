...
void TDC_Ck_Avg(Int_t run_number=5){ 
  ...
  Int_t adc_high_target[256];       Int_t ADC_High_TARGET[256];    
	...
	// Trigger
	fChain->SetBranchAddress("TDC_Trig", tdc_trig);
	...
	TH1D *h_TDC_Trig;		char Title_TDC_Trig[100];		char Name_TDC_Trig[100];
	TH1D *h_TDC_Ck_corr;	char Title_TDC_Ck_corr[100];	char Name_TDC_Ck_corr[100];

	vector <double> vec_Ck;	
	vec_Ck.clear();

	vector <int> vec_Target_ADC_LG;
	vec_Target_ADC_LG.clear();
	vector <int> vec_Target_ADC_LG_sorted;
	vec_Target_ADC_LG_sorted.clear();
	...
	sprintf(Title_TDC_Trig ,"TDC Trig (Run %d)", run_number);
	sprintf(Name_TDC_Trig,"TDC Trig");

	sprintf(Title_TDC_Ck_corr ,"Ck Corrected (Run %d)", run_number);
	sprintf(Name_TDC_Ck_corr,"Ck Corrected");
	...
	h_TDC_Average = new TH1D(Name_TDC_Average, Title_TDC_Average, 110, -10, 100);
	h_TDC_Diff = new TH1D(Name_TDC_Diff, Title_TDC_Diff, 500, 450, 950);
	h_TDC_Trig = new TH1D(Name_TDC_Trig, Title_TDC_Trig, 500, 1600, 2100);
	h_TDC_Ck_corr = new TH1D(Name_TDC_Ck_corr, Title_TDC_Ck_corr, 1000, 0, 1000);
	...
	//for (Int_t ivt=0; ivt<50000; ivt++){
		...
		//for(int i=0; i<2; i++){
		//	for(int j=0; j<16; j++){
		//		cout << ivt << "  " << i << "  " << j << "  " << tdc_trig[i][j] << endl;
		//	}
		//}

		//cout << ivt << "   " << tdc_trig[0][0] << endl;

		h_TDC_Trig->Fill(tdc_trig[0][0]);
  		...
  		for(int i = 0; i<256; i++){  
  			...
  			//vec_Target_ADC_LG.push_back(ADC_Low_TARGET[i]);
  		}
 
  		//vec_Target_ADC_LG_sorted = sort(vec_Target_ADC_LG.begin(), vec_Target_ADC_LG.end());
  		//sort(vec_Target_ADC_LG.begin(), vec_Target_ADC_LG.end(), greater<int>());

  		//for(unsigned int ff=0; ff<vec_Target_ADC_LG.size(); ff++){
  		//	cout << "ALBANE : " << vec_Target_ADC_LG[ff] << "   " << endl;
  		//}
	    ...
	    for(unsigned int i=0; i<vec_Ck.size(); i++){
  		...
  		//cout << "TEST : " << ivt << "   " << TDC_average << "   " << TDC_ck_avg2 << endl;

		h_TDC_Ck_Avg->Fill(TDC_ck_avg2);
		//h_TDC_Diff->Fill(TDC_ck_avg2-TDC_average);
		//h_TDC_Diff->Fill(TDC_ck_avg2);
		//h_TDC_Ck_corr->Fill(tdc_trig[0][0]-TDC_ck_avg2);
		h_TDC_Ck_corr->Fill(tdc_trig[0][0]);  // <----- why empty ?
