...
void SFT_angle_selector(int Run_Number, int ievt, double phi, int gap_to_fit);

void Event_Display_CR_Seb(Int_t Run_Number=5, Int_t ievt=0, Int_t enable_cout=9, Int_t display_fits = 0)
{ 
...
//  sprintf(h_target_TDC_title,"Run %d (Event %d) -- ADC #geq %d | %d #leq TDC #leq %d",Run_Number, ievt, par_temp_TARGET[1][0],TDC_min_TARGET,TDC_max_TARGET);
  ...
  for(int ivt=ievt; ivt<ievt+1; ivt++){
    ...
    cout << "  " << endl;
    for(int i=0; i<512; i++){
      if(MWPCADC[i]>0)  cout << "   " << i << "    " << MWPCADC[i] << endl;
    }
    cout << "  " << endl;

    for(int i=0; i<56; i++){
      if(ADC_C2X_R[i]>0) cout << "   " << " C2_X_" << i+1 << "_Right  " << ADC_C2X_R[i] << endl;
      if(ADC_C2X_L[i]>0) cout << "   " << " C2_X_" << i+1 << "_Left  " << ADC_C2X_L[i] << endl;
    }

    for(int i=0; i<64; i++){
      if(ADC_C3X_R[i]>0) cout << "   " << " C3_X_" << i+1 << "_Right  " << ADC_C3X_R[i] << endl;
      if(ADC_C3X_L[i]>0) cout << "   " << " C3_X_" << i+1 << "_Left  " << ADC_C3X_L[i] << endl;
    }

    for(int i=0; i<72; i++){
      if(ADC_C4X_R[i]>0) cout << "   " << " C4_X_" << i+1 << "_Right  " << ADC_C4X_R[i] << endl;
      if(ADC_C4X_L[i]>0) cout << "   " << " C4_X_" << i+1 << "_Left  " << ADC_C4X_L[i] << endl;
    }

     for(int i=0; i<16; i++){
      if(ADC_C2Y_R[i]>0) cout << "   " << " C2_Y_" << i+1 << "_Right  " << ADC_C2Y_R[i] << endl;
      if(ADC_C2Y_L[i]>0) cout << "   " << " C2_Y_" << i+1 << "_Left  " << ADC_C2Y_L[i] << endl;
      if(ADC_C3Y_R[i]>0) cout << "   " << " C3_Y_" << i+1 << "_Right  " << ADC_C3Y_R[i] << endl;
      if(ADC_C3Y_L[i]>0) cout << "   " << " C3_Y_" << i+1 << "_Left  " << ADC_C3Y_L[i] << endl;
      if(ADC_C4Y_R[i]>0) cout << "   " << " C4_Y_" << i+1 << "_Right  " << ADC_C4Y_R[i] << endl;
      if(ADC_C4Y_L[i]>0) cout << "   " << " C4_Y_" << i+1 << "_Left  " << ADC_C4Y_L[i] << endl;
    }
...
///////////////////////////////////////////////////////// K STOP CENTROID END
  ...
  if((gap_to_fit_rotate==12 || gap_to_fit_rotate==6) && Rotate==1){
    ...
    for(int i = 0; i < max_length; i++){
      ...
      for(int j = 0; j < max_length; j++){
        ...
        else if(gap_to_fit_rotate == 11 || gap_to_fit_rotate == 5){  // tof1 = 11/5
          x1_rotated = (sqrt(3.0)/2.0)*C2Y_L_centroids[i] - (1.0/2.0)*629.4;    // gap 11
          x2_rotated = -(sqrt(3.0)/2.0)*C2Y_R_centroids[j] + (1.0/2.0)*629.4;   // gap 5   //modified by Mike & Sebastien July 6
          y1_rotated = (1.0/2.0)*C2Y_L_centroids[i] + (sqrt(3.0)/2.0)*629.4;    // gap 11
          y2_rotated = -(1.0/2.0)*C2Y_R_centroids[j] - (sqrt(3.0)/2.0)*629.4;   // gap 5   // modified by Mike & Sebastien July 6
        ...
        else if(gap_to_fit_rotate == 1 || gap_to_fit_rotate == 7){
          x1_rotated = (sqrt(3.0)/2.0)*C2Y_R_centroids[i] + (1.0/2.0)*629.4;    // gap 1
          x2_rotated = -(sqrt(3.0)/2.0)*C2Y_L_centroids[j] - (1.0/2.0)*629.4;   // gap 7
          y1_rotated = -(1.0/2.0)*C2Y_R_centroids[i] + (sqrt(3.0)/2.0)*629.4;    // gap 1
          y2_rotated = (1.0/2.0)*C2Y_L_centroids[j] - (sqrt(3.0)/2.0)*629.4;   // gap 7
  ...
  for(int i = 0; i < c2xl_num_lines; i++){
    for(int j = 0; j < c2xr_num_lines; j++){
      if(gap_to_fit_rotate==1 || gap_to_fit_rotate==7){
      ...
      if(gap_to_fit_rotate==11 || gap_to_fit_rotate==5){
        x1_C2_coord = C2X_L_centroids[i];    // gsap 11
        x2_C2_coord = C2X_R_centroids[j];   // gap 5  
        y1_C2_coord = 629.4;    // gap 11
        y2_C2_coord = -629.4;   // gap 5  

        double C2_intersect_SFT_top = C2_intersect(points_to_slope(x1_C2_coord,y1_C2_coord,x2_C2_coord,y2_C2_coord),points_to_y_int(x1_C2_coord,y1_C2_coord,x2_C2_coord,y2_C2_coord),R_SFT_L1);
        double C2_intersect_SFT_bottom = C2_intersect(points_to_slope(x1_C2_coord,y1_C2_coord,x2_C2_coord,y2_C2_coord),points_to_y_int(x1_C2_coord,y1_C2_coord,x2_C2_coord,y2_C2_coord),-R_SFT_L1);
        ...
        x1_C2_coord = C2X_R_centroids[j];    // gap 1
        x2_C2_coord = C2X_L_centroids[i];   // gap 7
        y1_C2_coord = 629.4;    // gap 1
        y2_C2_coord = -629.4;   // gap 7
      ...
      else if(gap_to_fit_rotate==6 || gap_to_fit_rotate==12){
        double C2_intersect_SFT_top = C2_intersect(points_to_slope(C2X_L_centroids[i],629.4,C2X_R_centroids[j],-629.4),points_to_y_int(C2X_L_centroids[i],629.4,C2X_R_centroids[j],-629.4),R_SFT_L1);
        double C2_intersect_SFT_bottom = C2_intersect(points_to_slope(C2X_L_centroids[i],629.4,C2X_R_centroids[j],-629.4),points_to_y_int(C2X_L_centroids[i],629.4,C2X_R_centroids[j],-629.4),-R_SFT_L1);
  ...
  SFT_angle_selector(Run_Number,ievt,angle_final_guide, gap_to_fit_rotate);
...
void SFT_angle_selector(int Run_Number, int ievt, double phi, int gap_to_fit){
  ...
  if(gap_to_fit==5 || gap_to_fit==11){

    if(phi > 105 && phi < 135){ //gap 11
  ...
  else if(gap_to_fit==6 || gap_to_fit==12){

    if(phi > 70 && phi < 110){ //gap 12
  ...
  else if(gap_to_fit==1 || gap_to_fit==7){

    if(phi > 0 && phi < 100){ //gap 1
    ...
    cout << "Track not in TOF1-(11/5, 12/6, 7/1)" << endl;
  }
...
void SFT_Test_CR_Event(int Run_Number, int evt, double phi, int gap_to_fit){
  ...
  cout << "SFT Z Values -- -- TOF1 " << gap_to_fit << " -- -- Angle Used = " << phi << endl;
