...
double *calculate_min_Z_position(double phi, double p1, double p2, double p3, double p4, double return_array[20], int iteration_counter, double delta_z[100]){
  ...
  for(int h = 0; h<num_turns[0]; h++){
    if(p1 <= 0) no_p1_flag=true;

    for(int i = 0; i<num_turns[1]; i++){
      if(p2 <= 0) no_p2_flag=true;

      for(int j = 0; j<num_turns[2]; j++){
        if(p3 <= 0) no_p3_flag=true;

        for(int k = 0; k<num_turns[3]; k++){
          if(p4 <= 0) no_p4_flag=true;
                ...
                if(n[1] < smallest_dist){
                  smallest_dist_temp = n[1];
                   ...
                   if(!smallest_dist_flag){                       
                      ...
                      return_array[0] = (Z_positions[0][layer_1_low_index] 
                                    + Z_positions[3][layer_3_low_index])/2.0;  // OLD: + Z_positions[3][layer_4_low_index])/2.0; (mod. Oct. 24 2016)
                   }
                }
              }
...
vector<double> Z_list(double ADC_High_SFT_corr[128], int has_TDC_SFT_hit[128], int SFT_channel_to_fiber[128], 
  ...
  //cout << "DEBUG:  " << ZZ.size() << "  " << DZ.size() << "  " << Track_Length.size() << endl;
...
vector<double> Delta_Z_list(double ADC_High_SFT_corr[128], int has_TDC_SFT_hit[128], int SFT_channel_to_fiber[128],
 double phi, bool to_print, double TOF1_pos, bool use_TOF1, double C2X_centroid, double len_in_target){
...
vector<double> Track_Length_list(double ADC_High_SFT_corr[128], int has_TDC_SFT_hit[128], int SFT_channel_to_fiber[128],
 double phi, bool to_print, double TOF1_pos, bool use_TOF1, double C2X_centroid, double len_in_target){
