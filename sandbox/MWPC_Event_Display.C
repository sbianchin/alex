...
void MWPC_Event_Display(Int_t Run_Number=5, Int_t Event_Number=-1){ 
	...
	for(int i=0; i<14; i++){
		h_C2XL_M->Fill(i+1,C2X1L[i]);
		h_C2XL_M->Fill(i+15,C2X2L[i]);
		h_C2XL_M->Fill(i+29,C2X3L[i]);
		h_C2XL_M->Fill(i+43,C2X4L[i]);

		h_C2XR_M->Fill(i+1,C2X1R[i]);
		h_C2XR_M->Fill(i+15,C2X2R[i]);
		h_C2XR_M->Fill(i+29,C2X3R[i]);
		h_C2XR_M->Fill(i+43,C2X4R[i]);
	}
