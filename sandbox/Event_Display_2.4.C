...
void Event_Display_2_4(Int_t Run_Number=5, Int_t ievt=0, Int_t enable_cout=0, Int_t display_fits = 0){ 
  ...
  for(int ivt=ievt; ivt<ievt+1; ivt++){
    ...
    for(Int_t ii = 0; ii < nBars; ii++){
      ...
      //cout << ij << " " << ik << " " << par_in[ii][0] << " " << par_err[ii][0] << endl;
      parTARGdat >> ij >> ik >> par_in[ii][1] >> par_err[ii][1];
      //cout << ij << " " << ik << " " << par_in[ii][1] << " " << par_err[ii][1] << endl;
      parTARGdat >> ij >> ik >> par_in[ii][2] >> par_err[ii][2];
      //cout << ij << " " << ik << " " << par_in[ii][2] << " " << par_err[ii][2] << endl;
    }

    //cout << "DEBUG BEFORE : " << tdc_le_target[68][0] << endl; 

    for(Int_t ii = 0; ii<256; ii++){
      Yfi = par_in[ii][0] - par_in[ii][1]/sqrt(ADCmax - par_in[ii][2]);
      //cout << "DEBUG:  " << par_in[ii][0] << "  " << par_in[ii][1] << "  " << ADCmax << "  " << par_in[ii][2] << "  " << Yfi << endl;
      //Ani = adc_high_target[ii]-Target_HG_ped[ii]; //DAVE
      Ani = adc_high_target[ii]-TARGET_ADC_Thr_HG[ii]; //SEB

      //if((Ani >= Target_HG_ped[ii]) && (Ani < ADCmax)){ // DAVE
      if((Ani >= TARGET_ADC_Thr_HG[ii]) && (Ani < ADCmax)){ // SEB
        Yni = par_in[ii][0] - par_in[ii][1]/sqrt(Ani - par_in[ii][2]);
        Tpi = Yfi - Yni;
        //cout << "DEBUG:  " << tdc_le_target[ii][0] << "  " << Tpi << "  " << int(Tpi) << "  " << int(round(Tpi)) << endl;
        for(int jj=0; jj<16; jj++){
          //if(tdc_le_target[ii][jj]>0) TDCnew[ii][jj] = tdc_le_target[ii][jj] + Tpi; 
          if(tdc_le_target[ii][jj]>0) tdc_le_target[ii][jj] = tdc_le_target[ii][jj] + Tpi;
        }
      }
    }

    //cout << "DEBUG AFTER : " << tdc_le_target[68][0] << endl;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for(Int_t icol=0; icol<256; icol++){
    if (has_TDC_hit_Kstop[icol]) {
      ...
      if(ADC_Low_TARGET[icol]>LG_KAON && ADC_High_TARGET[icol]>HG_KAON){ 
        marker_ADCL_TARGET[icol]->SetMarkerColor(12); 
        marker_ADCL_TARGET[icol]->Draw();
 ...
 ///////////////////////////////////////////////////////// K STOP CENTROID END
  ...
  h_int_TDC_SFT->SetMarkerStyle(20);        h_int_TDC_TARGET->SetMarkerStyle(20);     h_int_TDC_TARGET_rotate->SetMarkerStyle(20);
  h_int_TDC_SFT->SetMarkerColor(1);         h_int_TDC_TARGET->SetMarkerColor(1);      h_int_TDC_TARGET_rotate->SetMarkerColor(1);
  h_int_TDC_SFT->SetMarkerSize(0.8);        h_int_TDC_TARGET->SetMarkerSize(0.8);     h_int_TDC_TARGET_rotate->SetMarkerSize(0.8);
  ...
  if((gap_to_fit_rotate==12 || gap_to_fit_rotate==6 || selected_TOF2==12 || selected_TOF2==6) && Rotate==1){ //ROTATE_CHANGE
    ...
    h_int_TDC_TARGET_rotate->Draw("same");
  ...
  cout << endl;
  cout << endl;
  //cout << "Gap1 : " << gap_to_fit_rotate << endl;
  //cout << "Gap2 : " << selected_TOF2 << endl;
  //cout << "X_TARGET: " << x_TARGET_intersect << "   " << "Y_TARGET: " << y_TARGET_intersect << endl;
  //TH2F *h_TARGET_TEST; 
  //TH2F *h_int_TDC_TARGET = new TH2F("h_int_TDC_TARGET", "h_int_TDC_TARGET", 500, -50, 50, 500, -50, 50);
