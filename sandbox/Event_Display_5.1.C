...
void Event_Display_5_1(Int_t Run_Number=5, Int_t ievt=0, int walk=0, Int_t enable_cout=0, Int_t display = 0){ 
  ...
  for(int ivt=ievt; ivt<ievt+1; ivt++){
    ...
    // CORRECT tdc vs. adc correlation (decorrelate)

    /////////////////////////////////////////////////////////////////////////////
      
    // SFT threshold correction
    ...
    // TOF1 threshold correction
    for(int i=0; i<12; i++){
      ADC_TOF1[i] = ADC_tof1U[i]-TOF1U_ADC_Thr[i]; // upstream
      ADC_TOF1[i+12] = ADC_tof1D[i]-TOF1D_ADC_Thr[i]; // downstream
    }
    // TOF2 threshold correction
    ...
    // Chambers threshold correction
    // 3 chambers, different numbers of wires for X, same numbers of wires for Y
      //value extracted from free, threshold extracted from #include
    ...
    for(int i=0; i<256; i++)
        // check first 4 TDC between window and ADC above 0
        // or even if High < 0 but Low > 0 then the event is still good
    {
      ...
      // good events have more than 5 good hits
  ...
  //// GAP SCORING ! Determine what gap is hit
  ...
  // Fill K Stop bars (in K stop Cuts fit)
  for(int i = 0; i<256; i++){
    if(k_stop_bar[i]){
  ...
  //for(int g=0; g<256; g++){
  //  if(distance_to_line(Xloc[g],Yloc[g],a_fit_kaon,b_fit_kaon)<=dist_to_kaon_fit){
  //    cout << "POE: " << g << "   " << distance_to_line(Xloc[g],Yloc[g],a_fit_kaon,b_fit_kaon) << endl;
  ...
  // Adding points for kaon track fit in Final Fit plot
  for(Int_t i=0; i<256; i++){
    ...
    // Add gap points and add Additional weight
  for(int g=0; g<12; g++){
  ...
  for(int i = 0; i<3; i++){
  //    cout << "SEB: " << Gap[gap_to_fit-1][1][0] << "    " << Gap[gap_to_fit-1][1][1] << endl;
