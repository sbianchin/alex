...
void Event_Display_3_4(Int_t Run_Number=5, Int_t ievt=0, int walk=0, Int_t enable_cout=0, Int_t display = 0){ 
  ...
  for(int ivt=ievt; ivt<ievt+1; ivt++){
    ...
    //cout << "ALBANE : " << tdc_le_target[99][0] << endl;
    ...
    if(walk==0){
      cout << "//////////////////////   WALK CORRECTION APPLIED !" << endl;
 ...
 /////////////////////////////////////////////////////////////////
  ...
  for(unsigned int i=0; i<vec_Ck.size(); i++){
    if(abs(vec_Ck[i]-TDC_ck_avg) <= 2*TDC_ck_sigma){
  ...
  for(unsigned int j=0; j<vec_Cpi.size(); j++){
    if(abs(vec_Cpi[j]-TDC_cpi_avg) <= 2*TDC_cpi_sigma){
  ...
  if ((xhyp1 <= xhyp2) && (xhyp1 <= xhyp3)) closest_gap_point = 0;
  else if (xhyp2 <= xhyp3) closest_gap_point = 1;
  else closest_gap_point = 2;
  ...
  double Gap[12][3][2] = {{{0}}};
  double a_fit_TDC_selected=0.;               float b_fit_TDC_selected=0.;
  ...
  //cout << "ALBANE: " << vec_xx_lepton.size() << endl;

  TGraph *gr3_Leptons = new TGraph(vec_xx_lepton.size(),&vec_xx_lepton[0],&vec_yy_lepton[0]);
  TF1 *gr_Leptons_fit3;
  gr3_Leptons->SetMarkerStyle(25);
  gr3_Leptons->SetMarkerColor(2);
  ...
  gr_Leptons_fit3 = gr3_Leptons->GetFunction("pol1");
  ...
  a_rotate = (-1)/(a_fit_TDC_Gap_Fibers);
  b_rotate = (-b_fit_TDC_Gap_Fibers)/(a_fit_TDC_Gap_Fibers);
  ...
  //cout << "PAULINE :  " << Gap[gap_to_fit-1][selected_TDC_selected][1] << "   " << Yth_TOF1 << endl;


  //cout << "DEBUG: " << a_fit_TDC_Gap_Fibers << "   " << a_rotate << "   " << 1/a_fit_TDC_Gap_Fibers << endl;
  ...
  if(X_BAR != -10000){
    ...
    if((gap_to_fit_rotate==6 || gap_to_fit_rotate==12 || selected_TOF2==12 || selected_TOF2==6) && Rotate==1){
      ...
      //cout << "OKK:  " <<x_TDC_Gap_Fibers_intersect1 << "   "
      //     << dist_to_k_stop << "   " << alpha << endl;
      //cout << "OKK:  " << x_tof1_intersect_1 << "  " << y_tof1_intersect_1 << endl;
