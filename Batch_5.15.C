...
void Batch_5_15(int Run_Number, int min_ievt, int max_ievt, int flag) {
  gROOT->SetBatch(1);
  ...
  if (flag == 0) {
    ...
    while (i < max_ievt) {
      ...
      Event_Display_5_4(Run_Number, i, 0, 0, 0, 0, 1, 1);
      ++i;
    }
  }
  else if (flag == 1) {
    int i = min_ievt;
    while (i < max_ievt) {
      Event_Display_5_4(Run_Number, i, 0, 0, 0, 1, 0, 1);
      ++i;
    }
  }
  else if (flag == 2) {
    ...
    while (fdat_read.good()) {
      ...
      fdat_read >> Read_ievt;
      cout << Read_ievt << endl;
      Event_Display_5_4(Run_Number, Read_ievt, 0, 0, 0, 0, 1, 1);
    }
  }
